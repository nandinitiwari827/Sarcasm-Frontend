...
import { forgotPassword, logoutUser } from "../../api.js"
    ...
    setErrors(newErrors)
       ...
       if (!isValid) return
        ...
        try {
          ...
          let res = await forgotPassword(email, phoneNumber, newPassword) 
    
          alert(res.message || "New password created successfully!")
          setEmail("")
          setPhoneNumber("")
          setNewPassword("")
          ...
          let backendError = err.response?.data?.message || "An error occured while changing password."
        
          if (status === 404) {
            setErrors({ email: "User does not exist", phoneNumber: "", newPassword: "" })
          } else {
            setErrors({ email: backendError, phoneNumber: "", newPassword: ""  })
          }
        } finally {
          setIsLoading(false)
        }}
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden w-full max-w-md">
        <div className="bg-indigo-600 py-6 px-6 flex flex-col items-center">
         ...
         <div className="flex w-full justify-between items-start">
          ...
          <h2 className="text-2xl md:text-4xl font-semibold text-white" style={{fontFamily: 'serif'}}>Forgot Password?</h2>
